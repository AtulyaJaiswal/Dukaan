...
exports.createDiscountCard = catchAsyncErrors(async (req, res, next) => {
  const { name, code, value, coins } = req.body;

  const card = await Discount.findOne({ code: code });

  if (card) {
    return next(new ErrorHandler(`Discount Card with this code already exist`));
  }
  ...
  await Discount.create({
    ...
    code,
    value,
    coinsNeededToRedeem: coins,
  });

  res.status(201).json({
    success: true,
    message: "Discount Card Created",
  });
});

//DELETE DISCOUNT CARD --ADMIN
exports.deleteDiscountCard = catchAsyncErrors(async (req, res, next) => {
  const discount = await Discount.findById(req.params.id);

  if (!discount) {
    return next(new ErrorHandler("Discount card not found", 404));
  }
...
//GET ALL DISCOUNT CARD (ADMIN)
exports.getAllDiscountCard = catchAsyncErrors(async (req, res, next) => {
