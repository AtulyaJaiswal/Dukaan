...
import { clearErrors, createCategory, getCategory } from '../../actions/productAction';
...
const Category = () => {
  ...
  const {loading,message,error} = useSelector((state) => state.newProductCategory);
  const {loading: detailsLoading, category, error: detailsError} = useSelector((state) => state.category);
